...
function getValue(tag, emv_objects, callback) {
  var found = emv_objects.filter(function (item) {
    if (item.tag == tag) callback(item.value);
  });
}
function getElement(tag, emv_objects, callback) {
  var found = emv_objects.filter(function (item) {
    if (item.tag == tag) callback(item);
  });
}
...
function parse(emv_data, callback) {
  ...
  while (emv_data.length > 0) {
    ...
    var tag = util
      .Bin2Hex(tag_class + tag_constructed + tag_number)
      .toUpperCase();
...
function unparse(emv_objects, callback, append_length = false) {
  let emv_data = "";
  for (let i = 0; i < emv_objects.length; i++) {
    let tag = emv_objects[i].tag;
    let len = emv_objects[i].length;
    let value = emv_objects[i].value;
    if (Array.isArray(value)) {
      value = unparse(value);
    }
    emv_data += `${tag}${len}${value}`;
  }
  if (append_length) {
    let length = (emv_data.length / 2)
      .toString(16)
      .toUpperCase()
      .padStart(4, "0");
    emv_data = length + emv_data;
  }
  callback(emv_data);
}
...
function aip(aip_data, callback) {
  ...
  var Byte1 = [
    ...
    {
      bit: "5",
      value: aip_bin.substring(3, 4),
      description: "Cardholder verification supported",
    },
    {
      bit: "4",
      value: aip_bin.substring(4, 5),
      description: "Terminal risk Management to be performed",
    },
    {
      bit: "3",
      value: aip_bin.substring(5, 6),
      description: "Issuer authentication supported",
    },
...
function cvm(cvm_data, callback) {
  ...
  if (cvm_code == "000000") data.push("Fail CVM processing");
  else if (cvm_code == "000001")
    data.push("Plaintext PIN verification performed by ICC");
  else if (cvm_code == "000010") data.push("Enciphered PIN verified online");
  ...
  else if (cvm_code == "011110") data.push("Signature (paper)");
  else if (cvm_code == "011111") data.push("No CVM required");
  ...
  if (condition_code == "00") data.push("Condition: Always");
  else if (condition_code == "01") data.push("Condition: If unattended cash");
  ...
  else if (condition_code == "04") data.push("Condition: If manual cash");
  else if (condition_code == "05")
    data.push("Condition: If purchase with cashback");
  else if (condition_code == "06")
    data.push(
      "Condition: If transaction is in the application currency 21 and is under X value (see section 10.5 for a discussion of “X”)"
    );
    ...
    data.push(
      "Condition: If transaction is in the application currency and is under Y value (see section 10.5 for a discussion of 'Y')"
    );
    ...
    util.Hex2Dec("0A") <= util.Hex2Dec(condition_code) &&
    util.Hex2Dec(condition_code) <= util.Hex2Dec("7F")
  )
    data.push("RFU");
  else if (
    util.Hex2Dec("80") <= util.Hex2Dec(condition_code) &&
    util.Hex2Dec(condition_code) <= util.Hex2Dec("FF")
  )
...
function auc(auc_data, callback) {
  ...
  var Byte1 = [
    {
      bit: "8",
      value: auc_bin.substring(0, 1),
      description: "Valid for domestic cash transactions",
    },
    {
      bit: "7",
      value: auc_bin.substring(1, 2),
      description: "Valid for international cash transactions",
    },
    {
      bit: "6",
      value: auc_bin.substring(2, 3),
      description: "Valid for domestic goods",
    },
    {
      bit: "5",
      value: auc_bin.substring(3, 4),
      description: "Valid for international goods",
    },
    {
      bit: "4",
      value: auc_bin.substring(4, 5),
      description: "Valid for domestic services",
    },
    {
      bit: "3",
      value: auc_bin.substring(5, 6),
      description: "Valid for international services",
    },
    { bit: "2", value: auc_bin.substring(6, 7), description: "Valid at ATMs" },
    {
      bit: "1",
      value: auc_bin.substring(7, 8),
      description: "Valid at terminals other than ATMs",
    },
  ];
  ...
  var Byte2 = [
    {
      bit: "8",
      value: auc_bin.substring(8, 9),
      description: "Domestic cashback allowed",
    },
    {
      bit: "7",
      value: auc_bin.substring(9, 10),
      description: "International cashback allowed",
    },
...
function tsi(tsi_data, callback) {
  ...
  var Byte1 = [
    {
      bit: "8",
      value: tsi_bin.substring(0, 1),
      description: "Offline data authentication was performed",
    },
    {
      bit: "7",
      value: tsi_bin.substring(1, 2),
      description: "Cardholder verification was performed",
    },
    {
      bit: "6",
      value: tsi_bin.substring(2, 3),
      description: "Card risk management was performed",
    },
    {
      bit: "5",
      value: tsi_bin.substring(3, 4),
      description: "Issuer authentication was performed",
    },
    {
      bit: "4",
      value: tsi_bin.substring(4, 5),
      description: "Terminal risk management was performed",
    },
    {
      bit: "3",
      value: tsi_bin.substring(5, 6),
      description: "Script processing was performed",
    },
...
function to_bits(hexData, callback) {
  ...
  for (var i = 0; i < no_bytes; i++) {
    ...
    for (var j = 0; j < 8; j++) {
      str +=
        " { bit : " + j + ", value : " + bin_data.substring(i, i + 1) + " }";
      if (j != 7) str += ",";
    }
    str += "]}";
    data.push(str);
...
const hex2dec = (hex, length) => {
  return hex.length === 6 ? hex : parseInt(hex.substr(0, length), 16);
};

function getConvertedValue(value, type, length) {
  switch (type) {
    case "ans":
      return hex2ascii(value);
    case "n":
      return hex2dec(value, length);
    case "an":
      return hex2ascii(value);
    case "b":
...
function parseCardType(card_type) {
  card_type = card_type?.toLowerCase();

  /**
   * if the card type contains interact its a debit card,
   * if the card type contains visa its a credit card,
   * if the card type contains mastercard its a credit card,
   * if the card type contains the visa or mastercard but also contains the debit keyword its a debit card,
   */
...
function getParsedEMV(encrypted) {
  let mapped_data = {};
...
module.exports = {
  parse: function (emv_data, callback) {
    parse(emv_data, callback);
  },
  unparse: function (emv_objects, callback, append_length) {
    unparse(emv_objects, callback, append_length);
  },
  describe: function (emv_data, callback) {
    describeKernel(emv_data, "Generic", callback);
  },
  lookup: function (emv_tag, callback) {
    lookupKernel(emv_tag, "Generic", callback);
  },
  describeKernel: function (emv_data, kernel, callback) {
    describeKernel(emv_data, kernel, callback);
  },
  lookupKernel: function (emv_tag, kernel, callback) {
    lookupKernel(emv_tag, kernel, callback);
  },
  getValue: function (emv_tag, emv_objects, callback) {
    getValue(emv_tag, emv_objects, callback);
  },
  getElement: function (emv_tag, emv_objects, callback) {
    getElement(emv_tag, emv_objects, callback);
  },
  aip: function (aip_data, callback) {
    aip(aip_data, callback);
  },
  auc: function (auc_data, callback) {
    auc(auc_data, callback);
  },
  cvm: function (cvm_data, callback) {
    cvm(cvm_data, callback);
  },
  tvr: function (tvr_data, callback) {
    tvr(tvr_data, callback);
  },
  tsi: function (tsi_data, callback) {
    tvr(tsi_data, callback);
  },
  // mappings,
  // getConvertedValue,
  // parseCardType,
